...
export const getBookstoreContract = async (): Promise<unknown> => {
  const networkId = await getNetworkId();
  //@ts-ignore
  return new web3.eth.Contract(BookstoreContract.abi, BookstoreContract.networks[networkId].address);
}

export const getBookstoreItemContract = async (): Promise<unknown> => {
  const networkId = await getNetworkId();
  //@ts-ignore
...
//test
console.log('Here!')
const result = getListedItems().then((value) => {
  console.log(value);
})
